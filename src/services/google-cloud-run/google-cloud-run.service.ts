...
@Injectable()
export class GoogleCloudRunService implements OnApplicationBootstrap {
  base_url: string;
  ...
  async onApplicationBootstrap(): Promise<any> {
    const connectorName = this.config.get<string>('name');

    try {
      this.base_url = await this.getCloudRunConnectorUrl(connectorName);
      this.registrationService.setCloudUrl(this.base_url);
  ...
  //     base_url: this.cloudRun.base_url,
